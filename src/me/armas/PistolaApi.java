...
public class PistolaApi {
	...
	public void getPistola(String modelo, Player p) {
		if (checkPistola(modelo)) {
			...
			Bukkit.getConsoleSender()
					.sendMessage("§cEl jugador " + p.getName() + " ha recibido el arma " + modelo + ".");
		} else {
			Bukkit.getConsoleSender().sendMessage(
					"§cEl modelo de arma que esta siendo seleccionado no existe, intÃ©ntelo de nuevo o compruebe el archivo de configuraciÃ³n a ver si todo esta correcto.");
			p.sendMessage("§c§l[ERROR]: §cCompruebe la consola para mas informaciÃ³n.");
		}
	}
	...
	public void disparaPistola(Player p) {
		...
		String name = p.getInventory().getItemInMainHand().getItemMeta().getDisplayName().replace("§", "&");
		String key = null;
		for (String _key : ArmasPlugin.get().getConfig().getConfigurationSection(ARMAS_PREFIX).getKeys(false)) {
			if(ArmasPlugin.get().getConfig()
					.getString(String.format(FRMT3, ARMAS_PREFIX,
							_key.toLowerCase(), NAME)).equals(name)) {
		...
		if (disparo.containsKey(p)) {
			...
			if (disparo.get(p) != 0) {
				int fuerza = ArmasPlugin.get().getConfig()
						.getInt(String.format(FRMT3, ARMAS_PREFIX, key, FUERZA));
			...
			disparo.put(p, ArmasPlugin.get().getConfig()
					.getInt(String.format(FRMT3, ARMAS_PREFIX, key, MAX)) - 1);
		}
	}
